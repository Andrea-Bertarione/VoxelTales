...
public class SetSkillCommand extends AbstractPlayerCommand {
    ...
    protected void execute(@NotNull CommandContext commandContext, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        try {
            VoxelWeaponHelper.selectSkill(playerRef, name.get(commandContext));
            playerRef.sendMessage(Message.raw("Skill set successfully to: " + VoxelSkillsRegistry.staticGetSkill(name.get(commandContext)).getDisplayName()));
        }
        catch (Exception err) {
            playerRef.sendMessage(Message.raw("Failed to set skill: " + err.getMessage()));
        }
    }
}
